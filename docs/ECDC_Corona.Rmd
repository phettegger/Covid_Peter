...
countries = c("AT", "CH", "DE", "FR", "IT", "UK", "TW")
...
ggplot(dat2, aes(x = Date, y = Cases, group = Country, colour = Country)) +
    geom_line() + scale_y_log10() + ylab("New cases")
...
ggplot(df.slopes, aes(x = Date, y = Doubling.Time, group = Country, colour = Country)) +
    geom_line() + ylab("Doubling Time / days") + xlab("Date")
